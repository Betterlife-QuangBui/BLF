...
var CommonTool = function () {
  ...
  this.DisplaySumCart = function () {
    ...
    for (var i = 0; i < tbCarts.length; i++) {
      var item = tbCarts[i]
      var yen = item.quantity * item.price_promotion
      var vnd = yen * exchange_rate
      sum_quantity += parseFloat(item.quantity)
      sum_yen += yen
      sum_vnd += vnd
    }
    document.querySelectorAll('.sum_quantity')[0].textContent = sum_quantity
    document.querySelectorAll('.sum_yen')[0].textContent = this.currency_format(sum_yen)
    document.querySelectorAll('.sum_vnd')[0].textContent = this.currency_format(sum_vnd)
  }
  ...
  this.spitTitle = function (title, length) {
  ...
  this.addProToCart = function (data) {
    ...
    for (var i = 0; i < tbCarts.length; i++) {
      ...
      if (item.key == shop_id) {
        ...
        for (var j = 0;j < arrShop.length;j++) {
          ...
          sum_yen += parseFloat(arrShop[j].yen_)
          sum_vnd += parseFloat(arrShop[j].vnd_)
        }
        if (flag == false) {
          arrShop.push(data)
          sum_quantity += parseFloat(data.quantity)
          sum_yen += parseFloat(data.yen_)
          sum_vnd += parseFloat(data.vnd_)
          tbCarts[i].value = arrShop
        }
    ...
    if (flagShop == false) {
      ...
      var jsonShop = {'shippingJ_price_yen': this.currency_format(shipping),'shippingJ_price_vnd': this.currency_format(shipping * exchange_rate),'key': shop_id,'shop_code': data.shop_code,'shop_name': data.shop_name,'shop_url': data.shop_url,'time': getDateTime(),'sum_quantity': data.quantity,'sum_yen': data.yen,'sum_vnd': data.vnd,'value': tbShops}
  ...
  this.showProInCart = function (data) {}
...
var AddonTool = function () {
  ...
  this.AddToCart = function () {
    ...
    var data = {
      ...
      title_:common_tool.spitTitle($.trim(title_origin),10),
      title_origin: $.trim(title_origin),
      price: price_promotion,
      // price_origin: price_origin,
      price_promotion: common_tool.currency_format(price_promotion),
      quantity: quantity,
      yen_: quantity * price_promotion,
      vnd_: quantity * price_promotion * exchange_rate,
...
$(document).on('click', '#myCart', function () {
  ...
  var foundin = $('*:contains("<!-- プレ画面TOP固定ヘッダー -->")')
  if (foundin) {
    $(foundin).next().hide()
    foundin.next().hide()
  }
...
$(document).on('change', '.cart_line', function () {
  ...
  $(this).find('.yen').html(common.currency_format(yen))

  $(this).find('.vnd').html(common.currency_format(vnd))
