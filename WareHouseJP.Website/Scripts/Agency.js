...
$(document).on("click", "#storejp-table .storejp-pagging-index a", function (e) {
    ...
    var code = $("#storejp-table thead tr input:eq(0)").val();
    var name = $("#storejp-table thead tr input:eq(1)").val();
    var address = $("#storejp-table thead tr input:eq(2)").val();
    var phone = $("#storejp-table thead tr input:eq(3)").val();
    var email = $("#storejp-table thead tr input:eq(4)").val();
    var hotline = $("#storejp-table thead tr input:eq(5)").val();
    var fax = $("#storejp-table thead tr input:eq(6)").val();
    var status = $("#storejp-table thead tr select:eq(0)").val();

    if (href === undefined || href === null) { }
    else {
        $("#storejp-table tbody").html("");
        href = href + "&code=" + code + "&name=" + name + "&address=" + address + "&phone=" + phone + "&email=" + email + "&hotline=" + hotline + "&fax="
...
$(document).on("click", "#storejp-table .sort-row .sort", function (e) {
    ...
    var href = "/Agency/AjaxIndex?page=1" + "&code=" + code + "&name=" + name + "&address=" + address + "&phone=" + phone + "&email=" + email + "&hotline=" + hotline + "&fax="
            + fax + "&status=" + status + "&data_sort=" + data_sort;
...
function deleteStore(storejp) {
    $.ajax({
        method: 'POST', async: false,
        url: "/Agency/DeleteMultiple/", data: { ids: storejp },
        success: function (result) {
            if (result.status == true) {
                notify('white', 'Xóa dữ liệu thành công', "1");
                $(".wasehouse-info,.package_edit_body,.package_add_body").html("");
