...
{
    public class WareHouseInfoController : ManagementSystemController
    {
        // GET: WareHouseInfo
        ...
        public ActionResult AjaxIndex(int page = 1, string name = "", string address = "", string phone = "", string email = "", string hotline = "", string fax = "", string status = "0", string data_sort = "")
        {
            ...
            #endregion
            var lstReturn = Pager<WareHouseInfo>.CreatePagging(item, page, 10);
            return PartialView("~/Views/WareHouseInfo/_ItemOfPage.cshtml", lstReturn);
        }
        [HttpPost]
        public ActionResult Delete(int id)
        {
            try
            {
                db.WareHouseInfoes.Remove(db.WareHouseInfoes.Find(id));
                db.SaveChanges();
        ...
        public ActionResult DeleteMultiple(string ids)
        {
            try
            {
                ...
                {
                    db.WareHouseInfoes.Remove(db.WareHouseInfoes.Find(int.Parse(id)));
                }
                db.SaveChanges();
                return Json(new { message = "Xóa dữ liệu thành công !", status = true }, JsonRequestBehavior.AllowGet);
            }
