var rakuten =  function(cart_url) {
    ...
    this.parse = function () {
        ...
        var price_html = '<p class="ohayoo-append  ohayoo-large" style="margin: 10px 0;">' +
            'Tỉ giá: ' + common.currency_format(common.getExchangeRate(),true)+' VNĐ / 1 JPY</p>';
    ...
    this.set_translate = function(data) {
        ...
        if(_title != null && data.title != ""){
            ...
            proid_append.parent().parent().parent().append("<tr><td colspan='2' class='ohayoo-append ohayoo-small'>Jan code:  " + document.getElementsByClassName("item_number")[0].textContent + "</td></tr>");
            time_append.parent().parent().parent().append("<tr><td colspan='2' class='ohayoo-append ohayoo-small'>Thời gian bán:  " + time_value + "</td></tr>");           
        }
    };
    ...
    this.getTimeSale = function(){
        try {
            ...
            if (document.getElementsByClassName('time_sale').length > 0) {
                time_sale = document.getElementsByClassName('time_sale')[1].textContent;
            }
            return time_sale;
        }catch(e) {
            return "";
