    ...
    this.translate_title = function (title, type, object) {
        if (isUsingSetting) {//Nếu sử dụng setting thì lấy giá trị ở setting
            ...
            if (isTranslate) {
                ...
                chrome.runtime.sendMessage({
                    action: "translate", url: translate_url, method: 'POST', data: { text: "販売期間	2016年05月01日10時00分～2016年08月30日09時59分" }, callback: 'afterTranslate'
...
var Action = {
    ...
    afterTranslate: function (request) {
        console.log(request);
        console.info("tam====afterTranslate");
        try {
            var object = new factory(cart_url, add_to_cart_url);
