!function($){ 'use strict';
    ...
    $.fn.dynamitable = function(options) {
        ...
        var dynamitableCore = new (function($dynamitable) {
             ...
             *
             * get a field value of input, select or text
             *
             * return string
             **********************************************/
            this.getValue = function(index, $row) {
                if (this.getField(index, $row).has('input, select').length > 0){
                    return this.getField(index, $row).children('input, select').val();
                }
                else {
        ...
        this.addFilter = function addFilter(selector) {
            ...
            var filterAction = function() {
                 ...
                 **********************************************/
                 ...
                 $("tr").filter("[data-trackingparent]").each(function (i, e) {
                     if($(this).css("display") != "none"){
                         var tracking = $(this).attr("data-trackingparent");
                         $("tr").filter("[data-mainparent]").each(function (i, e) {
                             if ($(this).attr("data-mainparent") == tracking) {
                                 $(this).show();
                             }
                         })
                     }
                 ...
                 for (var j = 0; j < dynamitable.find('tfoot td').length; j++) {
                     dynamitable.find('tfoot td')[j].style.bottom = "0px";
                 }

                /**********************************************
                 * End custom for blf-jp
                 **********************************************/
            };
