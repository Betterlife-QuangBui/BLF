$(document).on("change", "#ShippingStatus", function (e) {
    key = $("#txt-export-good").val();
    $(".package_add_body,.package_edit_body,.wasehouse-info,.package_content").html("");
    $(".package_content").load("/ajax/Shipping?key=" + key + "&page=1&sort=" + $(this).val());
    return false;
    e.preventDefault();
});
//add new package
...
$(document).on("keyup", "#txt-export-good", function (e) {
    key = $(this).val();
    $(".package_add_body,.package_edit_body,.wasehouse-info,.package_content").html("");
    $(".package_content").load("/ajax/Shipping?key=" + key + "&page=1&sort=" + $('#ShippingStatus').val());
    return false;
    e.preventDefault();
});
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.status == true) {
                    $("#txt-export-good").val("");
                    var href = "/ajax/Shipping?page=1&sort=" + $("#ShippingStatus").val();
                    $(".package_add_body,.package_edit_body,.wasehouse-info,.package_content").html("");
