...
$(document).on("click", "#storejp-table .storejp-pagging-index a", function (e) {
    ...
    var agency = $("#storejp-table thead tr select:eq(0)").val();
    var loginname = $("#storejp-table thead tr input:eq(0)").val();
    var fullname = $("#storejp-table thead tr input:eq(1)").val();
    var email = $("#storejp-table thead tr input:eq(2)").val();
    var phone = $("#storejp-table thead tr input:eq(3)").val();
    var gender = $("#storejp-table thead tr select:eq(1)").val();
    var status = $("#storejp-table thead tr select:eq(2)").val();
    var role = $("#storejp-table thead tr select:eq(3)").val();
    if (role == "") { role = "0"; }
    if (href === undefined || href === null) { }
    else {
        $("#storejp-table tbody").html("");
        href = href + "&agency=" + agency + "&role=" + role + "&loginname=" + loginname + "&fullname=" + fullname + "&email=" + email + "&phone=" + phone + "&gender="
            + gender + "&status=" + status + "&data_sort=" + data_sort;
...
$(document).on("click", "#storejp-table .sort-row .sort", function (e) {
    ...
    var href = "/AgencyAccount/AjaxIndex?page=1" + "&agency=" + agency + "&role=" + role + "&loginname=" + loginname + "&fullname=" + fullname + "&email=" + email + "&phone=" + phone + "&gender="
            + gender + "&status=" + status + "&data_sort=" + data_sort;
...
$(document).on("click", ".checkall", function (e) {
    ...
    if (arr.length > 0) {
        ...
        $("#AgencyAccountId").val(arr.toString());
    }
    else {
        $(".action-table").find("input[type=text],button,textarea,select").attr("disabled", "disabled");
        $("#AgencyAccountId").val("");
...
function deleteStore(storejp) {
    $.ajax({
        method: 'POST', async: false,
        url: "/AgencyAccount/DeleteMultiple/", data: { ids: storejp },
        success: function (result) {
            if (result.status == true) {
                notify('white', 'Xóa dữ liệu thành công', "1");
                $(".wasehouse-info,.package_edit_body,.package_add_body").html("");
