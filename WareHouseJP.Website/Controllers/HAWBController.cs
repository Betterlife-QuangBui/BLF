...
{
    public class HAWBController : ManagementSystemController
    {// GET: WareHouseInfo
        public ActionResult Index(int page = 1, string key = "", int sort = 0)
        {
            ViewBag.Title = "Danh sách HAWB";
            ViewBag.key = key;
            ViewBag.sort = sort;
            ViewBag.page = page;
            ...
            var item = db.HAWBs.OrderByDescending(n => n.CreatedAt);
            return View(Pager<HAWB>.CreatePagging(item, page, 10));
        }
        public ActionResult AjaxIndex(int page = 1, string agencyCode = "", string code = "", string name = "", string address = "", string phone = "", string email = "", string hotline = "", string fax = "", string status = "0", string data_sort = "")
        {
            ...
            #endregion
            var lstReturn = Pager<HAWB>.CreatePagging(item, page, 10);
            return PartialView("~/Views/HAWB/_ItemOfPage.cshtml", lstReturn);
        }
        [HttpPost]
        public ActionResult Delete(string id)
        {
            try
            {
                db.HAWBs.Remove(db.HAWBs.Find(id));
                db.SaveChanges();
