...
var CommonTool = function () {
    ...
    this.getHomeLand = function () {
        ...
        if (url.match(/auctions.yahoo.co.jp/)) {
            return "AUCTIONS";
        }
        if (url.match(/shopping.yahoo.co.jp/)) {
            return "SHOPPING";
        }
        return null;
    };
    ...
    this.processPrice = function (price, site) {
        ...
        var li_price = null;
        if (site == 'RAKUTEN') {
            li_price = '<tr style="font-weight:bold;color:blue;font-size:23px;"><td>Giá:</td><td>' + price_show + '</td></tr>';
            if ($("body").hasClass("double_price") == true) {
                $(".double_price").append(li_price);
            }
            else {
                $(".price2").parent().parent().parent().prepend(li_price);
            }
        }
    ...
    this.key_translate_lib = function (key) {
        var translate = [];
        translate['カラー'] = 'Màu';
        translate['大きさ'] = 'Kích cỡ';

        translate['価格'] = 'Giá';
        translate['昇進の'] = 'Khuyến mại';
        translate['出荷'] = 'Vận Chuyển';
        translate['量'] = 'Số Lượng';
        translate['ポリシー'] = 'Chính sách';
        translate['レビュー'] = 'Đánh Giá';
        translate['カラー'] = 'Màu sắc';

        translate['種類'] = 'Loại';
        translate['单价（元）'] = 'Giá (JPY)';
        translate['インベントリ'] = 'Tồn kho';
        translate['数量'] = 'SL mua';

        translate['品質'] = "Chất lượng";
        translate['15変更回答'] = "15 đổi trả";
        translate['48配送時間'] = "48 giờ giao hàng";
...
var rakuten = function (cart_url) {
    ...
    this.getPromotionPrice = function (site) {
        ...
        } catch (ex)
        {
            try
            {
                ...
                this.common_tool.processPrice(normal_price[0].textContent.match(/[0-9]*[\.,]?[0-9]+/g), site);
            } catch (ex) {}
        }
    };
    ...
    this.getTitleOrigin = function () {
        alert("");
        try {
            return document.getElementsByClassName("time_sale")[0].textContent;
        } catch (ex) {
            return "";
        }
    ...
    this.getDomTitle = function () {
        try {
            var _title = document.getElementsByClassName("time_sale")[0].textContent;
            return _title;
        } catch (ex) {
            return null;
        }
...
var Action = {
    ...
    afterTranslate: function (request) {
        try {
            ...
            object.set_translate({ title: request.response });
        } catch (ex) {
...
function start() {
    ...
    $(document).on('click', '._addToCart', function () {
        ...
        if (origin_site.match(/rakuten.co.jp/)) {
            object.add_to_cart();
        } else {
