...
$(document).on("click", "#myCart", function(){
    ...
    var common = new CommonTool();
    document.querySelectorAll(".ex_rate_money")[0].textContent = exchange_rate + " đ";
    var sum_quantity = 0;
    var sum_yen = 0;
    var sum_vnd = 0;
    $(".tbody").html("");
    for (var i = 0; i < tbCarts.length; i++) {
        ...
        tr += "<td><a href='" + item.link_origin + "' target='_blank'>" + item.title_origin + "</a></td>";
        //tr += "<td><a href='" + item.link_origin + "' target='_blank'>View</td>";
        tr += "<td>" + item.category_name + "</td>";
        tr += "<td>Size:<br><input type='text' class='form-control size'/><br>Màu:<br><input type='text' class='form-control color'/><br>Mô tả:<br><input type='text' class='form-control desc'/></td>";
        tr += "<td><input type='number' min='0' value='" + item.quantity + "'/></td>";
        tr += "<td>" + common.currency_format(item.price_promotion) + "</td>";
        tr += "<td>" + common.currency_format(yen) + "</td>";
        tr += "<td>" + common.currency_format(vnd) + "</td>";
        tr += "<td></td>";
        tr += "</tr>";
        $(".tbody").append(tr);
    }
    document.querySelectorAll(".sum_quantity")[0].textContent = sum_quantity;
    document.querySelectorAll(".sum_yen")[0].textContent = common.currency_format(sum_yen);
    document.querySelectorAll(".sum_vnd")[0].textContent = common.currency_format(sum_vnd);
...
$(document).on("click", "#btnExport", function () {
  var dtltbl = $('#cart_info').html();    
  window.open('data:application/vnd.ms-excel,' + $('#cart_info').html());
});