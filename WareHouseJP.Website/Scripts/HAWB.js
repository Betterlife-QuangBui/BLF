...
$(document).on("click", "#storejp-table .storejp-pagging-index a", function (e) {
    ...
    var agencyCode = $(".choose-agency select:eq(0)").val();
    ...
    if (href === undefined || href === null) { }
        ...
        href = href + "&agencyCode=" + agencyCode + "&code=" + code + "&name=" + name + "&address=" + address + "&phone=" + phone + "&email=" + email + "&hotline=" + hotline + "&fax="
...
$(document).on("click", "#storejp-table .sort-row .sort", function (e) {
    ...
    var href = "/HAWB/AjaxIndex?page=1" + "&agencyCode=" + agencyCode + "&code=" + code + "&name=" + name + "&address=" + address + "&phone=" + phone + "&email=" + email + "&hotline=" + hotline + "&fax="
            + fax + "&status=" + status + "&data_sort=" + data_sort;
...
$(document).on("change", "#storejp-table .filter-jp select,.choose-agency select", function (e) {
    $(".wasehouse-info,.package_edit_body,.package_add_body").html("");
    loading()
    var data_sort = $("#sort-setting").val();
    var agencyCode = $(".choose-agency select:eq(0)").val();
...
$(document).on("click", "a.package_item-delete-new", function (e) {
    if (confirm("Bạn có chắc muốn xóa HAWB này?")) {
        $.ajax({
...
function deleteStore(storejp) {
    $.ajax({
        method: 'POST', async: false,
        url: "/HAWB/DeleteMultiple/", data: { ids: storejp },
        success: function (result) {
            if (result.status == true) {
                notify('white', 'Xóa dữ liệu thành công', "1");
                $(".wasehouse-info,.package_edit_body,.package_add_body").html("");
