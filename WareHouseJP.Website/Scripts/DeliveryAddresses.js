...
$(document).on("click", "#storejp-table .storejp-pagging-index a", function (e) {
    ...
    var agencyCode = $("#storejp-table thead tr select:eq(0)").val();
    ...
    if (href === undefined || href === null) { }
        ...
        href = href + "&code=" + code + "&agencyCode=" + agencyCode + "&address=" + address +"&data_sort=" + data_sort;
...
$(document).on("click", "#storejp-table .sort-row .sort", function (e) {
    ...
    var href = "/DeliveryAddresses/AjaxIndex?page=1" + "&code=" + code + "&agencyCode=" + agencyCode + "&address=" + address + "&data_sort=" + data_sort;
...
$(document).on("click", ".checkall", function (e) {
    ...
    if (arr.length > 0) {
        ...
        $("#DeliveryAddressesId").val(arr.toString());
    }
    else {
        $(".action-table").find("input[type=text],button,textarea,select").attr("disabled", "disabled");
        $("#DeliveryAddressesId").val("");
    }
});
...
$(document).on("click", ".btn-action-table", function (e) {
    $(".se-pre-con").show();
    storejp = $("#DeliveryAddressesId").val();
...
function deleteStore(storejp) {
    $.ajax({
        method: 'POST', async: false,
        url: "/DeliveryAddresses/DeleteMultiple/", data: { ids: storejp },
        success: function (result) {
            if (result.status == true) {
                notify('white', 'Xóa dữ liệu thành công', "1");
                $(".wasehouse-info,.package_edit_body,.package_add_body").html("");
